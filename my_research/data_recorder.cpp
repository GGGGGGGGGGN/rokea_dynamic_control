...
int main() {
  ...
  // ==========================================
  robot.startMove(
      RCI::robot::StartMoveRequest::ControllerMode::kJointImpedance,
  ...
  // 核心 1kHz 实时回调函数
  // ==========================================
  auto joint_position_callback =
      [&](RCI::robot::RobotState robot_state) -> JointPositions {
    if (!is_init) {
  ...
  std::cout << "实时采集结束！正在将数据保存到文件，并进行链路分析..."
            << std::endl;
  ...
  // 写入 CSV 表头
  outfile << "msg_id,success_rate,q1,q2,q3,q4,q5,q6,q7,dq1,dq2,dq3,dq4,dq5,dq6,"
             "dq7,tau1,tau2,tau3,tau4,tau5,tau6,tau7\n";

  int drop_count = 0;
  for (size_t i = 0; i < data_log.size(); ++i) {
    auto &rd = data_log[i];

    // 统计丢包情况：如果这一次的心跳包编号比上一次的差值大于 1，说明中间有 UDP
    // 丢包
    if (i > 0) {
  ...
  std::cout << "总计接收数据帧数: " << data_log.size() << " 帧 (理论应为 "
            << MAX_RECORD_TICKS << ")" << std::endl;
  std::cout << "系统检测到丢包数: " << drop_count << " 帧" << std::endl;
  std::cout << "底层评估指令成功率: " << data_log.back().success_rate * 100
            << " %" << std::endl;
  std::cout << "================================================" << std::endl;
